...
fn find_leftmost(placements : &mut Vec<Placement>,
                 rect: BBox, line : Line) -> Point {
    // if line is empty
    if line[4] == -1 {
        let new_pos = (line[0] + rect.width, line[3]);
        placements.push(Placement{ bbox: rect, pos: new_pos, line});
        return new_pos;
    }
    else {
        // search placements in line
        let new_pos = placements.into_iter().filter(| place | place.line == line)
...
fn wasted_space(placements : &mut Vec<Placement>, lines: Vec<Line>)
    -> Vec<Placement> {
    // clone max width
    ...
    for pl in placements {
        for mut group in place_groups {
            if pl.line == last_placement.line {
                group.push(pl.to_owned());
                last_placement = pl.clone();
            }
            else{
                // break the group, add new one
                place_groups.push(vec!(pl.to_owned()))
            }
        }
    }
...
fn into_houses(inv_space : &mut Vec<Placement>) -> Houses {
    ...
    for space in inv_space.chunks(2) {
        ...
        if a_height != last_elem.line[2] {break_group = true;}
        else {last_group.push(space[0].clone());}
        if b_height != last_elem.line[2] {break_group = true;}
        else {last_group.push(space[1].clone());}
...
fn merge_houses(mut houses: Houses) -> Houses {
    // sort lines by area
    use std::cmp::Ordering;
    ...
    let mut house_b = houses.pop().unwrap();
    for mut house_a in houses {
...
pub fn packer(bboxes: &mut Vec<BBox>) -> (Point, Vec<(BBox, Point)>) {
    ...
    for rect in bboxes {
        ...
        let selected_line = search_lines(rect.clone(), &mut xlines)
            .unwrap_or_else(|| {
        ...
        xlines = grouped_houses.into_iter().flatten().map(|x| x.line).collect();
    }

    // remove line data to return as unused
    return ((xlines[0][2], xlines.last().unwrap()[4]),
             placements.into_iter().map(| place | ((place.bbox, place.pos))).collect());
}
